...
dynamic_programming <- function(env, transit, varphidx, ...) {
  ...
  for (t in seq(env$tau)) {
    for (i in seq(env$nSdx)) {
      ...
      pi_rand[t, idx] <- data.table::fcoalesce(Q[t, idx] / sum(Q[t, idx], na.rm = T), 0.0)
...
(varphidx <- sample(nrow(env$scndx), env$tau, replace = TRUE)) # Fix scenario
...
system_transition <- function(env, transit, pi, varphidx) {
  ...
  cost <- matrix(NA, nrow = env$tau, ncol = 1L)
  q    <- numeric(env$tau)
  ...
  for (t in seq(env$tau)) {
    ...
    sdx <- ((i - 1L) * env$nAdx + (j - 1L)) * env$nScen + k

    next_i  <- transit[sdx,1L]
    cost[t] <- transit[sdx,2L]
    q[t] <- env$actionSupport[j]

    S.I[idx+env$nI,1L] <- env$stateSupport[env$Sdx[next_i,env$I_]]
    S.J[jdx+env$nJ,1L] <- env$extendedStateSupport[env$Sdx[next_i,env$nI+env$J_]]
...
cbind2(cbind2(env$stateSupport[env$Sdx[,env$I_]], env$extendedStateSupport[env$Sdx[,env$nI+env$J_]]), V[t,]) |>
  as.data.frame() |>
...
plot_value_surface <- function(VT, t, theta = 30, phi = 30) {
  ...
  persp3D(x = env$stateSupport, y = env$extendedStateSupport, z = t(Vx), col = vertex_colors, theta = theta, phi = phi,
          xlab = "Entry State", ylab = "Exit State", zlab = "V", main = paste0("State Value Surface at t = ", t))

  # Add contours to the surface
  contour3D(x = env$stateSupport, y = env$extendedStateSupport, z = t(Vx), colvar = t(Vx), add = TRUE, col = "darkblue", colkey = FALSE, lwd = 2)
}

## Plot the action value surface at time t
norm_Q <- (Q[t, Qdx] - min(Q[t, Qdx], na.rm = T)) / (max(Q[t, Qdx], na.rm = T) - min(Q[t, Qdx], na.rm = T))

vertex_colors <- create_gradient_palette(1000L, sort(norm_Q))

persp3D(x = env$extendedStateSupport, y = env$actionSupport, z = t(Qx), col = vertex_colors, theta = -45, phi = 30,
        xlab = "Exit State", ylab = "Action", zlab = "Q", main = "State Value Surface")

contour3D(x = env$extendedStateSupport, y = env$actionSupport, z = t(Qx), colvar = t(Qx), add = TRUE, col = "darkblue", colkey = FALSE, lwd = 2)
...
for (t_ in seq(env$tau + 1L)) {
  ...
  text(# x = c(-1.34,-1.34,-1.34, 1.34, 1.34,-1.00, -1.00,-0.70,  1.00, 1.00), 
    # y = c(-1.00, 0.00, 1.00,-1.00, 0.00,-0.60,  0.40, 1.20, -0.65, 0.35), 
    x = coords[[setoo]]$x,
    y = coords[[setoo]]$y,
    labels = c(
