...
Eigen::VectorXd multivariateNormalSample(const Eigen::VectorXd& mean, const Eigen::MatrixXd& L, std::mt19937& gen) {
...
Eigen::MatrixXd rmvnorm(int n, const Eigen::VectorXd& mean, const Eigen::MatrixXd& covar, int nThreads = 8) {
  ...
  if (cholSolver.info() != Eigen::Success) Rcpp::stop("Cholesky decomposition failed");
